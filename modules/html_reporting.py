...
TRANSLATIONS = {
    ...
    'cz': {
        ...
        'language': 'Seznam citlivých slov',
    ...
    <div class="container">
        ...
        <footer>
            <p style="display: flex; align-items: center; justify-content: center; margin: 0;">
                <span>Created by <strong>Docs-X-Ray</strong> tool by San-Tus</span>
                <a href="https://github.com/San-Tus/Docs-X-Ray" target="_blank" style="display: inline-flex; align-items: center; margin-left: 8px; color: #e6e6e6; text-decoration: none;">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" style="margin-right: 4px;">
...
def _get_css_styles() -> str:
    ...
    return """
        * {
        ...
        .term-preview-badge {
            display: inline-block;
            background: rgba(33, 155, 149, 0.15);
            color: #0c0c14;
            padding: 4px 10px;
            font-size: 0.75em;
            font-weight: 600;
            border-radius: 3px;
            border: 1px solid rgba(33, 155, 149, 0.3);
        }
...
def _generate_files_section(scan_results: List[Dict[str, Any]], report_lang: str) -> str:
    ...
    for item in scan_results:
        ...
        if is_clean:
            ...
            # Collect all unique terms found in this file with their counts
            all_terms = []
            for category in results.values():
                for word, data in category.items():
                    count = len(data['pages'])
                    all_terms.append((word, count))

            # Create a compact preview of terms (limit to first 5)
            if all_terms:
                preview_terms = all_terms[:5]
                terms_badges = ' '.join([
                    f'<span class="term-preview-badge">{escape_html(term)} <span class="term-count-mini">{count}</span></span>'
                    for term, count in preview_terms
                ])
                more_indicator = f' <span class="more-terms">+{len(all_terms) - 5} {t("more_terms")}</span>' if len(all_terms) > 5 else ''
                terms_preview = f'<div class="terms-preview">{terms_badges}{more_indicator}</div>'
            else:
                terms_preview = ''
